...
func GenerateAccessToken(userID uuid.UUID, pathSecret string, expiry int) (string, error) {
...
func GenerateRefreshToken(userID uuid.UUID, pathSecret string, expiry int) (string, error) {
	privateKeyBytes, errPath := os.ReadFile(pathSecret)
	if errPath != nil {
		return "", errPath
	}
